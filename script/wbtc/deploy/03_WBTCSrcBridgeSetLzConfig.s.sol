...
contract WBTCSrcBridgeSetLzConfig is ConfigSetup {
    ...
    function run() public {
        ...
        _run(true, srcLzEndpoint, srcLzSendUlnLib, srcLzSendUlnConfig, srcLzRecvUlnLib, srcLzRecvGracePeriod, srcLzRecvUlnConfig, srcWBTCBridge, destEID, destLzReceiveGas);
    }
    ...
    function _run(
        ...
        address _srcWBTCBridge,
        uint32 _destEID,
        uint128 _destLzReceiveGas
    ) public {
        if (broadcast) vm.startBroadcast();

        ILayerZeroEndpointV2(_srcLzEndpoint).setSendLibrary(_srcWBTCBridge, _destEID, _srcLzSendUlnLib);

        SetConfigParam[] memory params = new SetConfigParam[](1);
        params[0] = SetConfigParam(_destEID, ULN_CONFIG_TYPE, abi.encode(_srcLzSendUlnConfig));
        ILayerZeroEndpointV2(_srcLzEndpoint).setConfig(_srcWBTCBridge, _srcLzSendUlnLib, params);

        ILayerZeroEndpointV2(_srcLzEndpoint).setReceiveLibrary(_srcWBTCBridge, _destEID, _srcLzRecvUlnLib, _srcLzRecvGracePeriod);

        params[0] = SetConfigParam(_destEID, ULN_CONFIG_TYPE, abi.encode(_srcLzRecvUlnConfig));
        ILayerZeroEndpointV2(_srcLzEndpoint).setConfig(_srcWBTCBridge, _srcLzRecvUlnLib, params);
        ...
        WBTCOFTAdapter(_srcWBTCBridge).setEnforcedOptions(options);
