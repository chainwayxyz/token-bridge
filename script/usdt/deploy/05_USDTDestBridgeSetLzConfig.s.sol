...
contract USDTDestBridgeSetLzConfig is ConfigSetup {
    ...
    function run() public {
        ...
        _run(true, destLzEndpoint, destLzSendUlnLib, destLzSendUlnConfig, destLzRecvUlnLib, destLzRecvGracePeriod, destLzRecvUlnConfig, destUSDTBridgeProxy, srcEID, srcLzReceiveGas);
    }
    ...
    function _run(
        ...
        ILayerZeroEndpointV2(_destLzEndpoint).setSendLibrary(_destUSDTBridgeProxy, _srcEID, _destLzSendUlnLib);

        SetConfigParam[] memory params = new SetConfigParam[](1);
        params[0] = SetConfigParam(_srcEID, ULN_CONFIG_TYPE, abi.encode(_destLzSendUlnConfig));
        ILayerZeroEndpointV2(_destLzEndpoint).setConfig(_destUSDTBridgeProxy, _destLzSendUlnLib, params);

        ILayerZeroEndpointV2(_destLzEndpoint).setReceiveLibrary(_destUSDTBridgeProxy, _srcEID, _destLzRecvUlnLib, _destLzRecvGracePeriod);

        params[0] = SetConfigParam(_srcEID, ULN_CONFIG_TYPE, abi.encode(_destLzRecvUlnConfig));
        ILayerZeroEndpointV2(_destLzEndpoint).setConfig(_destUSDTBridgeProxy, _destLzRecvUlnLib, params);
        ...
        DestinationOUSDT(_destUSDTBridgeProxy).setEnforcedOptions(options);
